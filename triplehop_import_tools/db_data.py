...
async def create_entities(
    ...
    entity_type_id = await db_structure.get_entity_type_id(pool, params['project_name'], params['entity_type_name'])
...
async def create_relations(
    ...
    project_id = await db_structure.get_project_id(pool, params['project_name'])
    relation_type_id = await db_structure.get_relation_type_id(pool, params['project_name'], params['relation_type_name'])
    ...
    for row in batch:
        ...
        for d_prop_value in d_prop_values:
            ...
            if d_prop_value not in lookups[d_entity_type_name][d_prop_name]:
                ...
                continue
            domain_prop_values.append(lookups[d_entity_type_name][d_prop_name][d_prop_value])
        ...
        for r_prop_value in r_prop_values:
            ...
            if r_prop_value not in lookups[r_entity_type_name][r_prop_name]:
                ...
                continue
            range_prop_values.append(lookups[r_entity_type_name][r_prop_name][r_prop_value])
    ...
    for placeholder in props_collection:
            ...
            [
                {
                    ...
                    'properties': json.dumps({k:params[k] for k in params if k not in ['domain_id', 'range_id']}),
                }
        ...
        await db_base.executemany(
            ...
            [{'params': json.dumps({'id': params['id']})} for params in props_collection[placeholder]],
            True,
        )
...
async def create_lookup(
    ...
    if type == 'entity':
        entity_type_id = await db_structure.get_entity_type_id(pool, project_name, type_name)

        if prop_name == 'id':
            key = 'id'
        else:
            db_props_lookup = await get_entity_props_lookup(pool, project_name, type_name)
        ...
        return {
            json.loads(record['prop']): record['id']
            for record in records
        }
    else:
        relation_type_id = await db_structure.get_relation_type_id(pool, project_name, type_name)
        records = await db_base.fetch(
            pool,
            (
                    f'SELECT * FROM cypher('
...
async def create_entity_index(
    ...
    project_id = await db_structure.get_project_id(pool, project_name)
    entity_type_id = await db_structure.get_entity_type_id(pool, project_name, entity_type_name)
...
async def create_relation_entity_index(
    ...
    project_id = await db_structure.get_project_id(pool, project_name)
    relation_type_id = await db_structure.get_relation_type_id(pool, project_name, relation_type_name)
...
async def delete_source_relations(
    ...
    query = (
        f'DELETE FROM "{project_id}"._source_;'
    )
    try:
        await db_base.execute(
            pool,
...
async def create_entity_source_relations(
    ...
    for row in batch:
        ...
        props_lookup = await get_entity_props_lookup(pool, project_name, row['entity_type'])
        ...
        for p in row['properties']:
            ...
            if m:
                uuid_props.append(f'{props_lookup[m.group("property")]}[{m.group("index")}]')
            else:
...
async def create_relation_source_relations(
    ...
    for row in batch:
        ...
        if f'r_{rt}' not in lookups:
            lookups[f'r_{rt}'] = await create_lookup(pool, project_name, rt, 'id', 'relation')
        ...
        # Create lookup to convert property system names to property ids
        props_lookup = await get_relation_props_lookup(pool, project_name, row['relation_type'])
